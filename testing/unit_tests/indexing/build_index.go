...
// indv test cases
var Indvs = []interface{}{
	&donations.Individual{ID: "indv00", Name: "JOHNSON, BOB", City: "Cincinnati", State: "OH"},
	&donations.Individual{ID: "indv01", Name: "WILLIAMS, CHRISTINA", City: "New York", State: "NY"},
	&donations.Individual{ID: "indv02", Name: "DORAME ZAMBADA, JOSE", City: "Chicago", State: "IL"},
	&donations.Individual{ID: "indv03", Name: "HICKS, ANDRE", City: "Vallejo", State: "CA"},
	&donations.Individual{ID: "indv04", Name: "BROADUS, CALVIN", City: "Long Beach", State: "CA"},
}
...
func TestComponents() {
	...
	nw := 0
	tc := Indvs
	// tc = append(tc, &donations.Individual{})  // edge case
	// tc = []interface{}{}  // edge case
	sds := createSearchData(year, tc)
	id := &IndexData{
		Size:        0,
	...
	fmt.Println("index data start: ", id)
	fmt.Println()
	...
	for _, sd := range sds {
		...
		fmt.Println("formatted: ", termsFmt)
		for _, term := range termsFmt {
			nw++
			p := getPartition(term)
			fmt.Printf("\tpartition: %s\tterm: %s\n", p, term)
		}
	}

	updateIndexData(id, bucket, nw)
	fmt.Println("index data end: ", id)
	fmt.Println()
}

// SUCCESS
func TestGetObjData() error {
	year := "2018"
	index := make(indexMap)
	...
	if err != nil {
		fmt.Println(err)
		return fmt.Errorf("TestGetObjectData failed: %v", err)
	}
	...
	if err != nil {
		fmt.Println(err)
		return fmt.Errorf("TestGetObjData failed: %v", err)
	}
	fmt.Println("items: ", len(index))
...
// SUCCESS
func TestBuildIndex() error {
	year := "2018"
	err := BuildIndex(year)
	if err != nil {
		fmt.Println(err)
		return fmt.Errorf("TestBuildIndex failed: %v", err)
	}
	return nil
}

func TestUpdateIndex() error {
	year := "2018"
	bucket := "candidates"
	err := UpdateIndex(year, bucket)
	if err != nil {
		fmt.Println(err)
		return fmt.Errorf("TestUpdateIndex failed: %v", err)
	}
