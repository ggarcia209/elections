...
// SUCCESS
func TestGetResults() error {
	// q0 := Query{Text: "Schiff", UserID: "007", TimeStamp: time.Now()}
	// q1 := Query{Text: "Schiff for  congress ", UserID: "007", TimeStamp: time.Now()}
	// q2 := Query{Text: "Adam Schiff", UserID: "007", TimeStamp: time.Now()}
	// q3 := Query{}
	q4 := Query{Text: "  jalskjdfh    ", UserID: "007", TimeStamp: time.Now()}
	results, err := GetResults(q4)
	if err != nil {
		fmt.Println(err)
		return fmt.Errorf("TestGetResults failed: %v", err)
	}
